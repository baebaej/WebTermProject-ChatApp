		...
		if (this.props.user == this.props.curruser) {
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						'div',
						{ className: 'messageidbox' },
						React.createElement('img', { src: 'img/user.png', width: '20', height: '20', alt: 'userlogo' }),
						this.props.user
					),
					React.createElement(
						'div',
						...
						React.createElement(
							'div',
							{ className: 'messagetimebox' },
							this.props.time
						),
						React.createElement(
							'div',
							{ className: 'messagecontextbox' },
							React.createElement(
								'span',
								null,
								this.props.text
			...
			return React.createElement(
				...
				React.createElement(
					'div',
					{ className: 'elsemessage' },
					React.createElement(
						'div',
						{ className: 'elsemessageidbox' },
						React.createElement('img', { src: 'img/user.png', width: '20', height: '20', alt: 'userlogo' }),
						this.props.user
					),
					React.createElement(
						'div',
						{ className: 'elsemessagebox' },
						React.createElement(
							'div',
							{ className: 'elsemessagecontextbox' },
							React.createElement(
								'span',
								null,
								this.props.text
...
var EmoticonList = React.createClass({
	displayName: 'EmoticonList',

	togglePicker: function togglePicker() {
		if (this.state.isOpen) this.setState({ isOpen: false });else this.setState({ isOpen: true });
	},

	handleSelect: function handleSelect(emoji) {
				...
				React.createElement(
					...
					React.createElement(
						...
						emojis.map(function (emoji) {
							return React.createElement(
								'li',
								{ key: emoji, onClick: function () {
										return _this2.handleSelect(emoji);
									} },
								emoji
							);
						})
...
var SelectRoom = React.createClass({
	...
	roomlist_init: function roomlist_init(data) {
		//var {rooms} = data;
	...
	handleSearchChange: function handleSearchChange(event) {
		...
		var filteredRooms = this.state.rooms.filter(function (room) {
			return room.name.toLowerCase() === searchTerm;
		});
		//console.log(filteredRooms);
		this.setState({ searchTerm: searchTerm, filteredRooms: filteredRooms });
	},

	handleNewRoom: function handleNewRoom(roomname) {
	...
	render: function render() {
		...
		var filteredRooms = _state3.filteredRooms;
		var selectedRoom = _state3.selectedRoom;

		var roomsToDisplay = searchTerm ? filteredRooms : this.state.rooms;
		...
		return React.createElement(
			...
			roomsToDisplay.map(function (room) {
				return React.createElement(
					'p',
					...
					React.createElement(
						...
						React.createElement(
							...
							'최근 메시지:',
							room.messages.length > 0 && React.createElement(
								'span',
								null,
			...
			!roomsToDisplay.length && React.createElement(
				'p',
				{ style: { textAlign: 'center' } },
				'채팅방이 없습니다.',
				React.createElement('br', null),
			...
			selectedRoom && React.createElement(
				'p',
				{ style: { textAlign: 'center' } },
				'현재 선택된 방: ',
				this.state.rooms.find(function (room) {
...
var AppFrame = React.createClass({
	...
	handleEnterChat: function handleEnterChat(username) {
	...
	handleSetUsers: function handleSetUsers(userlist) {
	...
	renderContent: function renderContent() {
		switch (this.state.selectedMenu) {
			case 1:
				return React.createElement(UsersScreen, { userlist: this.state.users });
			case 2:
				return React.createElement(ChatApp, { username: this.state.user, users: this.state.user });
			case 3:
				return React.createElement(ChangeNameForm, { onChangeName: this.handleChangeName, currentname: this.state.user });
		}
	},
	...
	render: function render() {
		...
		if (!this.state.enteredChat) {
			return React.createElement(LoginScreen, { onEnterChat: this.handleEnterChat, handleSetUsers: this.handleSetUsers });
		} else {
			return React.createElement(
				'div',
				{ className: 'appframe' },
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'div',
							{ className: 'idbox' },
							React.createElement('img', { style: { height: '50px', width: '50px' }, src: 'img/user.png', alt: '사용자' }),
							React.createElement(
								'p',
								null,
								'아이디 : ',
...
var MenuFrame = React.createClass({
	displayName: 'MenuFrame',

	handleFriendMenuClick: function handleFriendMenuClick() {
		this.props.onMenuChange(1);
	},

	handleChatMenuClick: function handleChatMenuClick() {
		this.props.onMenuChange(2);
	},

	handleUserMenuClick: function handleUserMenuClick() {
	...
	render: function render() {
		return React.createElement(
			'div',
			{ className: 'menuframe' },
			React.createElement('img', { className: 'menu-icon', onClick: this.handleFriendMenuClick, src: 'img/notSelected_friend.png', alt: '친구 메뉴' }),
			React.createElement('br', null),
			React.createElement('img', { className: 'menu-icon', onClick: this.handleChatMenuClick, src: 'img/selected_chat.png', alt: '채팅 메뉴' }),
			React.createElement('br', null),
			React.createElement('img', { className: 'menu-icon', onClick: this.handleUserMenuClick, src: 'img/notSelected_user.png', alt: '유저 메뉴' }),
			React.createElement('br', null)
		);
	}
});
...
var ChatApp = React.createClass({
	...
	_initialize: function _initialize(data) {
		...
		this.setState({ user: this.props.username, messages: messages, roomid: roomid, roomname: roomname, users: users });
	...
	handleEmoticonbtnClick: function handleEmoticonbtnClick() {
		if (this.state.emoticionbtnClicked) this.setState({ emoticionbtnClicked: false });else this.setState({ emoticionbtnClicked: true });
	},

	renderContent: function renderContent() {
		if (this.state.emoticionbtnClicked == true) return React.createElement(EmoticonList, {
	...
	render: function render() {
		...
		return React.createElement(
			...
			React.createElement(
				'div',
				{ className: 'roomNotSelectedContainer' },
				React.createElement('img', { className: 'roomNotSelected', src: 'img/selectroomplz.jpg' })
			) : //방이 선택된 경우
			React.createElement(
				'div',
				{ className: 'center' },
				React.createElement(
					'div',
					{ className: 'chattitleframe' },
					React.createElement(RoomTitle, { roomname: this.state.roomname }),
				...
				React.createElement(
					...
					React.createElement(MessageList, { messages: this.state.messages, curruser: this.state.user }),
...
React.render(React.createElement(AppFrame, null), document.getElementById('app'));
